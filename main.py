"""
Main Python File you can use to excute the program.
"""
from tkinter import *
...
def gettingEmpDetails():
    ...
    sql = "insert into employee values ('"+empid+"','"+name+"','"+password+"','"+department+"','"+doj+"')" 
    try:
        cur.execute(sql)
        con.commit()
    except:
...
def gettingStuDetails():
    ...
    sql = "insert into student values ('"+rollno+"','"+name+"','"+password+"','"+department+"','"+semester+"','"+batch+"')" 
    try:
        cur.execute(sql)
        con.commit()
    except:
...
def gettingLoginDetails():
    ...
    if (role == 'emp'):
        sqlLoginID = "select empid from employee where password = '"+password+"'"
        sqlName = "select name from employee where password = '"+password+"'"
        ...
        sqlLoginID = "select rollno from student where password = '"+password+"'"
        sqlName = "select name from student where password = '"+password+"'"
...
headingLabel = Label(headingFrame2, text="Welcome to Library", fg='black')
