"""
Python file to IssueBooks to student.
"""
from tkinter import *
...
def issue():
    ...
    extractBid = "select book_id from books"
    try:
        ...
        if book_id in allBid:
            checkAvail = "select status from books where book_id = '"+book_id+"'"
    ...
    extractRollno = "select rollno from student"
    try:
    ...
    extractEmpID = "select empid from employee"
    try:
    ...
    issueSql = "insert into book_issue values ('"+book_id+"','"+issue_to+"','"+issue_by+"')"
    show = "select * from book_issue"
    
    updateStatus = "update books set status = 'issued' where book_id = '"+book_id+"'"
    try:
        if book_id in allBid and issue_to in allRoll and issue_by in allEmpId and status == True:
